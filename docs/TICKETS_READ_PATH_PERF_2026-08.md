...
**Status:** Wave A+B engine complete; RP-02 ops open; RP-08 deferred  
**Date:** 2026-08-07 (docs refreshed for 0.6.x)  
...
| 2 | **RP-02** QSP shadow → route rollout + hydrate validation | M (ops + code) | Highest for hot lists | — |
...
| 8 | **RP-08** Generated projected columns (M1) for legacy surfaces | L | High long-term | RP-04 preferred first |
...
Multi-key keyset; `direction: 'before'` (F-01). GraphQL connection helpers still app-layer.
...
| `docs/RFC_MATERIALIZED_READ_MODELS.md` | Mark M1 implemented / link. |
...
| ID | Item | Notes |
|----|------|--------|
| F-01 | Multi-key keyset + `direction: 'before'` | Extends RP-06 |
...
| F-11 | Wire `startReconcileSweep` from App when QSP ≠ off | Ops gap today — documented as manual in QSP_OPERATIONS |

Update `docs/READ_PATH_PERFORMANCE.md` and `docs/QUERY_LIST_GUIDE.md` when F-07…F-11 land.
