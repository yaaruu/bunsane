...
| 9 | **SEC-09** GraphQL recon surface (introspection/GraphiQL/complexity) | M | Medium (recon + DoS) | — | **Done** | 2026-09-24 overhaul |
| 10 | **SEC-10** Info endpoints (`/metrics`, `/health`, Swagger) | S | Medium (internals disclosure, write amplification) | — | **Done** | 2026-09-24 overhaul |
...
**Status: Done (2026-09-24).** Introspection and GraphiQL are off unless `NODE_ENV=development` or `GRAPHQL_INTROSPECTION` / `GRAPHQL_GRAPHIQL=on` (or the App setters). Complexity uses coerced `first`/`limit`/`take`, charges fragment use sites, caps aliases at 50, and costs `__` fields at 10. Residual: a pure-introspection operation still skips the depth rule so tooling can run when introspection is explicitly on.
...
**Status: Done (2026-09-24).** `/metrics` and `/health/remote` are 404 without `BUNSANE_METRICS_TOKEN` (min 16) or `BUNSANE_METRICS=public`. `/docs`, `/docs/swagger-init.js`, and `/openapi.json` use `BUNSANE_DOCS_TOKEN` / `BUNSANE_DOCS=public` the same way. `/health` responses omit uptime and per-check latency. The write probe stays the default; `BUNSANE_HEALTH_PROBE=read` skips it. Probes are cached (`BUNSANE_HEALTH_CACHE_MS`, default 5s) and shed at `BUNSANE_HEALTH_MAX_RPS` (default 20) with 429. Swagger UI assets are pinned with SRI.
...
**Status: Done (2026-09-24).** Messages are HMAC-SHA256 signed with `BUNSANE_CACHE_INVALIDATION_SECRET` and rejected outside a timestamp window. Unset does **not** auto-generate a secret: pub/sub stays disabled and a **warn** is logged at startup (not info). Unsigned invalidation is not accepted.
...
**Status: Done (2026-09-24).** `BUNSANE_RPC_SECRET` signs RPC request/response, direct emit, and outbox envelopes. Unset keeps unsigned legacy envelopes and logs one startup warning. Set rejects unsigned or tampered envelopes (fail-closed); counters are `security.signatureRejected`, `replyToRejected`, and `duplicateDropped`. `replyTo` must be `rpc:responses:<instanceId>` or the message is ACK-dropped before the handler. Outbox publish is at-least-once; consumers drop a duplicate `(sourceApp, correlationId)` for 10 minutes in memory. `sourceApp` is stamped from server config. Trust-model text is in `docs/CONFIGURATION.md`.
...
**Status: Done (2026-09-24).** `invalidatePattern` requires a literal prefix, caps matches at `BUNSANE_CACHE_INVALIDATE_MAX` (default 10000), and aborts without deleting when the cap is exceeded. MemoryCache uses the same cap. Redis stats count own-prefix keys instead of `DBSIZE` when a key prefix is set.
...
**Status: Done (2026-09-24).** `securityHeaders` and `requestId` are registered in `start()` unless opted out. HSTS requires `BUNSANE_HSTS=on` or `BUNSANE_TLS=on` (`NODE_ENV=production` does not imply it). Studio and docs HTML send a CSP; Swagger assets use SRI.
