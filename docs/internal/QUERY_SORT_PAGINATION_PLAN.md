...
**Later (2026-09-24, F-01 partial):** single-key `sortedCursor(token, 'before')` no longer throws for entity-column sort, OR + one `sortBy`, or a single component `sortBy`. Multi-key still throws. OR + `sortedCursor` is supported for one sort key.
