...
| `DB_CONNECTION_TIMEOUT` | `30` (s) | How long the pool waits for a free connection before throwing `ERR_POSTGRES_CONNECTION_TIMEOUT`, which the framework answers with **503 + `Retry-After`** (`code: POOL_EXHAUSTED`) and counts as `poolAcquireFailures` in `/metrics`. **Request-facing deployments should set `5`.** The default stays 30 s because background work (scheduler, outbox, projection backfill/reconcile) shares this pool and legitimately waits longer — there is no per-lane timeout yet. |
