...
| `BUNSANE_ENTITY_SORT_PROBE` | `5000` | Upper bound for the `sortByCreatedAt` / `sortByUpdatedAt` + `.with()` probe. The probe walks entities in index order and checks membership; its window is sized from table statistics: `min(cap, max(64, ceil(4 × pageLimit / f)))`, where `f` is the component's share of all entities (`pg_class.reltuples`, cached 60 s). If the page fills, that is the answer (`getLastRouteInfo().entitySortPlan = 'probe'`); if it misses, or the estimate exceeds the cap, the hash-join + top-N plan runs (`'fallback'`). Bounds the cost when a component's entities are rare or clustered in time. Positive integer; invalid values fail boot. `OFFSET > 0` skips the probe. |
