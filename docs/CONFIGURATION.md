...
| `BUNSANE_QSP_ARCHETYPES` | (empty) | Optional CSV of archetype names. **Empty/unset:** every archetype is eligible, and the first covered list query starts backfill. **Set:** `App.init()` pre-registers those names as `DISABLED` and does not backfill them. Call `runBackfill` from `bunsane/database/projection` (see `docs/QSP_OPERATIONS.md`). |
...
`BUNSANE_QSP` defaults to `off`; with it unset at boot, `InitializeProjections` does not create `projection_state`. Enabling it later in the same process does not start projection or the reconcile sweep — restart. With it `shadow` or `route` at boot, an **unset** `BUNSANE_QSP_ARCHETYPES` lets the first covered list query create the read model and drive `BACKFILLING → SHADOW → READY` (there is no `NONE`). A set CSV pre-registers those archetypes as `DISABLED` and does not start that path. Status values are `DISABLED`, `BACKFILLING`, `SHADOW`, `READY`. See `docs/QSP_OPERATIONS.md`. Replaces the removed `BUNSANE_QSP_ENABLED` + `BUNSANE_QSP_MODE` pair.
