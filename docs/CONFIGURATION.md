...
| `DB_POOL_IDLE_TIMEOUT` | `30` (**s**) | Close idle pooled connections after this long. Bun SQL pool timeouts are in **seconds**. A value above `3600` is rejected at boot as a ms/s mix-up. Use `0` for no limit. |
| `DB_POOL_MAX_LIFETIME` | `600` (**s**) | Retire a pooled connection after this long regardless of activity. Recycling is the only mechanism that ever retires a connection the driver still believes is usable — before 0.5.11 these two were passed as milliseconds, so nothing was ever recycled. A value above `86400` is rejected at boot. `0` for no limit. The idle-timeout ceiling is `3600`, not this one. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `HEALTH_DB_WRITE_PROBE` | `true` (on) | Inside `deepHealthCheck`, run a real pooled write: `db.transaction()` on the lazy client inserts into a temp table and drops it on commit. It does not take an admission permit and does not emit `SET LOCAL statement_timeout`. `Entity.save` without a caller transaction uses the gateway `dbTransaction` instead, so a wedge inside admission is not what this probe catches. Set `false` to fall back to read-only `SELECT 1`. `BUNSANE_HEALTH_PROBE=read` skips the write probe at the HTTP endpoints before this flag is consulted. |
...
| `BUNSANE_INDEX_SYNC_MAX_ROWS` | `100000` | Key-index builds (`bk_*`, see below) run synchronously during `init()` for tables estimated below this row count; larger tables are built by a background task after `init()` under `withLock("bunsane:index-reconcile")`, so one instance builds and boot is not blocked. Unanalyzed tables fall back to size (> 64 MB = background). Non-negative integer; invalid values fail boot. Shutdown does not wait past the grace budget for an in-flight build (the server finishes it; the next boot repairs an invalid one). |
| `BUNSANE_ENTITY_SORT_PROBE` | `5000` | Cap for the single-key `sortByCreatedAt` or `sortByUpdatedAt` probe when the query also has membership (`.with()`, OR, or `.without()`). The window is `min(cap, max(64, ceil(4 × pageLimit / f)))`, where `f` is the component's share of all entities (`pg_class.reltuples`, cached 60 s). The probe is kept (`entitySortPlan = 'probe'`) when the window is exhausted or the page is full. Hash-join + top-N (`'fallback'`) runs only when the window was not exhausted and the page did not fill, or when the estimate exceeds the cap. `OFFSET > 0` skips the probe. Two entity sorts together with membership never probe (`'fallback'`). Without membership the plan is `'index'`. Positive integer; invalid values fail boot. |
...
**Key indexes.** Every `@CompData({ indexed: true })` scalar and `@IndexedField("btree" | "numeric")` field gets `bk_<slug>_<hash>` on its component leaf: `((key), entity_id)` (not partial, so the planner keeps expression statistics), where the key is `data->>'f'` or `bunsane_num_v1(data->>'f')` (an IMMUTABLE numeric-or-NULL cast created at boot). `@CompositeIndex(["status", "total"])` on a component adds `(status, total, entity_id)`. `entities` gets `created_at` / `updated_at` keys (UTC milliseconds). One index serves equality, range, sort in either direction with either NULLS placement, and keyset pages. Sorting by a field without a key index still works (full scan + top-N) and logs a one-time warning in development. The reconciler drops the legacy `idx_<leaf>_<f>_btree|_btree_date|_numeric` (and scalar `_gin`) indexes once their `bk_` replacement is valid, rebuilds invalid `bk_` indexes, and never touches indexes without the `bk_` prefix. Design: `docs/internal/RFC_INDEX_DRIVEN_LISTS.md`.
...
|----------|---------|--------|
| `BUNSANE_QSP` | `off` | single master knob: `off` (zero footprint, byte-identical to pre-QSP) \| `shadow` (project + verify parity, never serve rm_) \| `route` (project + shadow + promote to serving). `shadow` ↔ `route` and any → `off` are read per query. Turning QSP **on** after a boot that had it `off` needs a restart (`projection_state` and the reconcile sweep are created only in `App.init()`). |
| `BUNSANE_QSP_ARCHETYPES` | (empty) | Optional CSV of archetype names. **Empty/unset:** every archetype is eligible, and the first covered list query starts backfill. **Set:** `App.init()` creates the read model for each listed archetype and starts its backfill at boot (a *new* `projection_state` row goes straight to `BACKFILLING`, same as the unscoped lazy path); an existing row keeps its status, so `DISABLED` only happens if something set it by hand. See `docs/QSP_OPERATIONS.md`. |
...
| `BUNSANE_QSP_ENTITIES_ACCEL` | `false` | Reserved. `validateEnv` accepts `true` or `false`, but no runtime code reads it. There is no entities accelerator. `getLastRouteInfo().surface` is only `'rm'` or `'legacy'`. |
...
`BUNSANE_QSP` defaults to `off`; with it unset at boot, `InitializeProjections` does not create `projection_state`. Enabling it later in the same process does not start projection or the reconcile sweep — restart. With it `shadow` or `route` at boot, `BUNSANE_QSP_ARCHETYPES` only changes *when* a new archetype starts backfilling: unset lets the first covered list query create the read model and drive `BACKFILLING → SHADOW → READY` (there is no `NONE`); set, `App.init()` does the same for each listed archetype immediately, in the background. Either way a row's status survives restarts once it exists — `DISABLED` is reached only by an explicit rollback, never inserted automatically. Status values are `DISABLED`, `BACKFILLING`, `SHADOW`, `READY`. See `docs/QSP_OPERATIONS.md`. Replaces the removed `BUNSANE_QSP_ENABLED` + `BUNSANE_QSP_MODE` pair.
...
**List pagination:** on the component-table path, only an explicit `.take(N)` fetches `LIMIT N+1` and sets `query.getLastRouteInfo().hasNextPage`. A routed QSP `exec` (`surface: 'rm'`) always fetches `limit+1` when a limit is set, including the default `BUNSANE_DEFAULT_QUERY_LIMIT` applied before routing, so an unbounded covered query can report `hasNextPage: true` without `.take()`. Exact `.count()` remains available when called. Plain `.cursor(entityId)` cannot be combined with `.sortBy()`, `.sortByCreatedAt()`, or `.sortByUpdatedAt()` — use `.sortedCursor(token)` (`docs/READ_PATH_PERFORMANCE.md` §8).
...
|------------|---------------------|
| `test:pg` | `tests/unit` + `tests/integration` + `tests/graphql` + `tests/public-api` |
...
The `/health` endpoint exercises a **real pooled write** (a temp-table
`INSERT` inside `db.transaction()` on the lazy client, dropped on commit — no persistent side
effect). That is not the gateway `dbTransaction` path `Entity.save` uses: the probe takes no admission permit and sets no server `statement_timeout`. A read-only
