...
export function ArcheType() {
  ...
  const [data, setData] = useState<ArcheTypeRecord[]>([])
  ...
  const columns: ColumnDef<ArcheTypeRecord>[] = [
    ...
    ...archeTypeFields.map(field => ({
      accessorKey: field.componentName,
      header: field.fieldLabel || field.fieldName,
      cell: ({ getValue }: any) => {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-primary mb-2">{name} Archetype</h1>
        <p className="text-muted-foreground">
          Browse and manage records for the {name} archetype
        </p>
      </div>
